...
.. note:: 
	If the `KeepLayer <../setup/setup.html#Keeplayer>`_ attribute is set to ``no`` for an individual layer, it will not be added to the map even if the user selects a 'Yes' option in the dropdown list.
